-- ============================================================================
-- AdvancedMD Integration - Phase 3: Electronic Claim Submission (FIXED RLS)
-- ============================================================================
...
-- EDI Batches: Administrators and billing staff can manage batches
CREATE POLICY "Administrators and billing staff can manage EDI batches" ON advancedmd_edi_batches
  FOR ALL
  TO authenticated
  USING (
...
-- Batch Claims: Administrators and billing staff can manage
CREATE POLICY "Administrators and billing staff can manage batch claims" ON advancedmd_batch_claims
  FOR ALL
  TO authenticated
  USING (
    has_role(auth.uid(), 'administrator'::app_role)
...
-- Appeals: Administrators and billing staff can manage appeals
CREATE POLICY "Administrators and billing staff can manage appeals" ON advancedmd_claim_appeals
  FOR ALL
  TO authenticated
  USING (
    has_role(auth.uid(), 'administrator'::app_role)
...
-- Corrections: Administrators and billing staff can manage corrections
CREATE POLICY "Administrators and billing staff can manage corrections" ON advancedmd_claim_corrections
  FOR ALL
  TO authenticated
  USING (
    has_role(auth.uid(), 'administrator'::app_role)
...
-- CPT Codes: Everyone can read, only administrators can modify
CREATE POLICY "Authenticated users can read CPT codes" ON cpt_codes
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Administrators can manage CPT codes" ON cpt_codes
  FOR ALL
  TO authenticated
  USING (has_role(auth.uid(), 'administrator'::app_role))
  WITH CHECK (has_role(auth.uid(), 'administrator'::app_role));

-- ICD-10 Codes: Everyone can read, only administrators can modify
CREATE POLICY "Authenticated users can read ICD-10 codes" ON icd10_codes
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Administrators can manage ICD-10 codes" ON icd10_codes
  FOR ALL
  TO authenticated
  USING (has_role(auth.uid(), 'administrator'::app_role))
  WITH CHECK (has_role(auth.uid(), 'administrator'::app_role));

-- Denial Codes: Everyone can read, only administrators can modify
CREATE POLICY "Authenticated users can read denial codes" ON denial_codes
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Administrators can manage denial codes" ON denial_codes
  FOR ALL
  TO authenticated
  USING (has_role(auth.uid(), 'administrator'::app_role))
  WITH CHECK (has_role(auth.uid(), 'administrator'::app_role));
